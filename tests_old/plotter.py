...
#colorblind colors 
ibm_colors = {
    "Blue 40": "#5596E6",   # Soft blue
    "Purple 40": "#9855D4", # Mid-purple
    "Magenta 40": "#E71D73",# Deep pink/magenta
    "Orange 40": "#FF6F00", # Vivid orange
    "Yellow 40": "#FDD600"  # Bright yellow
}
...
def lossVng(phc,gme,alphas,ng,phcOG,gmeOG,alphasOG,ngOG,crystal):
    ...
    STYLE = {
        ...
        'colors': {
            'optimized': ibm_colors['Orange 40'],  # orange
            'original': ibm_colors['Purple 40']    # blue
        },
        'line': {
            'width': 3
        },
...
phcMid,gmeMid,alphasMid,ngMid = runSims(np.array(out[6]['x_values']),ZIW,out[-1])

with open('/Users/dominic/Desktop/optGME/tests/media/ginds3/W1Best.json','r') as file:
    out = json.load(file)

phcMidW1,gmeMidW1,alphasMidW1,ngMidW1 = runSims(np.array(out[5]['x_values']),W1,out[-1])
...
LINEWIDTH = 2.5
MARKER_SIZE = 6
STAR_SIZE = 15  # Size for star markers
...
# Add star markers for key points in W1 optimization
# Start point (index 0)
ax1.plot(1, W1optPlot[0], 's', markersize=STAR_SIZE, color=ibm_colors["Purple 40"], markeredgecolor='black', markeredgewidth=1, zorder=4)
# Middle point (6th point, index 5)
ax1.plot(6, W1optPlot[5], 's', markersize=STAR_SIZE, color=ibm_colors["Blue 40"], markeredgecolor='black', markeredgewidth=1, zorder=4)
# End point (last point)
ax1.plot(len(W1optPlot), W1optPlot[-1], 's', markersize=STAR_SIZE, color=ibm_colors["Orange 40"], markeredgecolor='black', markeredgewidth=1, zorder=4)
...
ax1.text(len(W1optPlot)*0.1, initial_value_w1*1.2, f'Original: {W1opt[0]:.5f}', 
         verticalalignment='bottom', horizontalalignment='right', fontsize=FONT_SIZE_ANNOTATION, 
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
ax1.text(len(W1optPlot)*0.1, final_value_w1*0.8, f'Optimized: {W1opt[-1]:.5f}', 
         verticalalignment='top', horizontalalignment='right', fontsize=FONT_SIZE_ANNOTATION,
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))

ax1.text(1.25, initial_value_w1*1.8, f'W1', 
         verticalalignment='bottom', horizontalalignment='left', fontsize=FONT_SIZE_TITLE, 
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
...
# Add star markers for key points in ZIW optimization
# Start point (index 0)
ax2.plot(1, ZIWoptPlot[0], 's', markersize=STAR_SIZE, color=ibm_colors["Purple 40"], markeredgecolor='black', markeredgewidth=1, zorder=4)
# Middle point (7th point, index 6)
ax2.plot(7, ZIWoptPlot[6], 's', markersize=STAR_SIZE, color=ibm_colors["Blue 40"], markeredgecolor='black', markeredgewidth=1, zorder=4)
# End point (last point)
ax2.plot(len(ZIWoptPlot), ZIWoptPlot[-1], 's', markersize=STAR_SIZE, color=ibm_colors["Orange 40"], markeredgecolor='black', markeredgewidth=1, zorder=4)
...
ax2.text(len(ZIWoptPlot)*0.85, initial_value_ziw*1.2, f'Original: {ZIWopt[0]:.5f}', 
         verticalalignment='bottom', horizontalalignment='right', fontsize=FONT_SIZE_ANNOTATION,
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
ax2.text(len(ZIWoptPlot)*0.85, final_value_ziw*0.75, f'Optimized: {ZIWopt[-1]:.5f}', 
         verticalalignment='top', horizontalalignment='right', fontsize=FONT_SIZE_ANNOTATION,
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
ax2.text(1.25, initial_value_ziw*7, f'ZIW', 
         verticalalignment='bottom', horizontalalignment='left', fontsize=FONT_SIZE_TITLE, 
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
...
phcW1mid = W1(vars=np.array(outW1[5]['x_values']))
phcZIWmid = ZIW(vars=np.array(outZIW[6]['x_values']))
phcW1OG = W1(NyChange=0)
phcZIWOG = ZIW(NyChange=0)

shapesW1 = phcW1OG.layers[0].shapes
shapesZIW = phcZIWOG.layers[0].shapes
shapesW1Mid = phcW1mid.layers[0].shapes
shapesZIWMid = phcZIWmid.layers[0].shapes
...
grey_level = 0.85  # Adjust this value between 0 (black) and 1 (white) for different grey shades
...
for c in shapesW1:
    ...
    circle = plt.Circle((c.y_cent, c.x_cent), c.r, facecolor='white', edgecolor=ibm_colors["Purple 40"],linewidth=4, alpha=1.0, zorder=1)
    ax.add_patch(circle)
    
    # Repeat circles in negative x direction (now vertical direction after flip)
    circle = plt.Circle((c.y_cent, c.x_cent - 1), c.r, facecolor='white', edgecolor=ibm_colors["Purple 40"],linewidth=4, alpha=1.0, zorder=1)
    ax.add_patch(circle)
    
    # Repeat circles in positive direction to show additional 3 unit copies
    for i in range(1, 3):
        circle = plt.Circle((c.y_cent, c.x_cent + i), c.r, facecolor='white', edgecolor=ibm_colors["Purple 40"],linewidth=4, alpha=1.0, zorder=1)
        ax.add_patch(circle)

# Plot the circles from the mid W1 structure as green dotted circles
for c in shapesW1Mid:
    # Create a circle with flipped x,y coordinates
    circle = plt.Circle((c.y_cent, c.x_cent), c.r, facecolor='none', edgecolor=ibm_colors["Blue 40"], alpha=1.0, linewidth=4, zorder=3)
    ax.add_patch(circle)
    
    # Repeat circles in negative x direction
    circle = plt.Circle((c.y_cent, c.x_cent - 1), c.r, facecolor='none', edgecolor=ibm_colors["Blue 40"], alpha=1.0, linewidth=4, zorder=3)
    ax.add_patch(circle)
    
    # Repeat circles in positive direction to show additional 3 unit copies
    for i in range(1, 3):
        circle = plt.Circle((c.y_cent, c.x_cent + i), c.r, facecolor='none', edgecolor=ibm_colors["Blue 40"], alpha=1.0, linewidth=4, zorder=3)
        ax.add_patch(circle)
    
# Plot the circles from the optimized W1 structure as red dashed circles
for c in shapesW1Opt:
    # Create a circle with flipped x,y coordinates
    circle = plt.Circle((c.y_cent, c.x_cent), c.r, facecolor='none', edgecolor=ibm_colors["Orange 40"], alpha=1.0, linewidth=4, zorder=4)
    ax.add_patch(circle)
    
    # Repeat circles in negative x direction
    circle = plt.Circle((c.y_cent, c.x_cent - 1), c.r, facecolor='none', edgecolor=ibm_colors["Orange 40"], alpha=1.0, linewidth=4, zorder=4)
    ax.add_patch(circle)
    
    # Repeat circles in positive direction to show additional 3 unit copies
    for i in range(1, 3):
        circle = plt.Circle((c.y_cent, c.x_cent + i), c.r, facecolor='none', edgecolor=ibm_colors["Orange 40"], alpha=1.0, linewidth=4, zorder=4)
        ax.add_patch(circle)
...
rect = plt.Rectangle((-4.25, -0.45), 1.7, 1.35, facecolor='white', edgecolor='black', alpha=0.8, zorder=5)
ax.add_patch(rect)
...
ax.text(x, y+0.8,
        'Original',           # two lines
        fontsize=28,                 # overall font size
        color=ibm_colors["Purple 40"],               # base color for the second line
        ha='center', va='center', fontweight='bold',
        zorder=6)

# Override the first line with red
ax.text(x, y + 0,  # small offset to move it above the second line
        'Optimized',
        fontsize=28,
        color=ibm_colors["Orange 40"],
        ha='center', va='center', fontweight='bold',
        zorder=6)

ax.text(x, y + 0.4,  # small offset to move it above the second line
        'Middle',
        fontsize=28,
        color=ibm_colors["Blue 40"],
        ha='center', va='center', fontweight='bold',
        zorder=6)
...
# Define the zigzag boundaries for the darker region
x_left = -np.sqrt(3)*1.55+np.sqrt(3)/2/6
x_right = np.sqrt(3)*1.55+np.sqrt(3)/2/6

# Create polygon for darker region
darker_poly = plt.Polygon([(x_left,-1),(x_right,-1),(x_right,5),(x_left,5)], closed=True, facecolor='grey', alpha=0.5, zorder=0.5)
ax.add_patch(darker_poly)
...
# Plot the circles from the mid ZIW structure as green dotted circles
for c in shapesZIWMid:
    # Create a circle with flipped x,y coordinates
    circle = plt.Circle((c.y_cent, c.x_cent), c.r, facecolor='none', edgecolor=ibm_colors["Blue 40"], alpha=1.0, linewidth=4, zorder=3)
    ax.add_patch(circle)
...
# Plot the circles from the optimized ZIW structure as orange dashed circles
...
# Add a small xy axis indicator in the lower left
# First create a white background for the axis without border
rect = plt.Rectangle((-4.1, -0.45), 1.7, 1.35, facecolor='white', edgecolor='black', alpha=0.8, zorder=5)
ax.add_patch(rect)
...
# Override the first line with orange
ax.text(x, y + 0,  # small offset to move it above the second line
        'Optimized',
        fontsize=28,
        color=ibm_colors["Orange 40"],
...
ax.text(x, y + 0.4,  # small offset to move it above the second line
        ...
        ha='center', va='center', fontweight='bold',
        zorder=6)
offset = np.sqrt(3)/2/6
plt.xlim(-5*np.sqrt(3)/2+offset, 5*np.sqrt(3)/2+offset)
plt.show()
# %%
with open('/Users/dominic/Desktop/optGME/tests/media/ginds3/ziwBest.json','r') as file:
    outZIW = json.load(file)
...
for i, val in enumerate(W1optPlot):
    idx = i + 1  # Adjust for 1-based indexing in plot
    if W1CV[i] > 0:
        ax1.plot(idx, val, 'ro', markersize=MARKER_SIZE, zorder=3, markeredgecolor=ibm_colors["Magenta 40"], fillstyle='none')
    else:
        ax1.plot(idx, val, 'o', markersize=MARKER_SIZE, markeredgecolor='darkblue', zorder=3, fillstyle='none')
...
ax1.text(len(W1optPlot)*0.1, initial_value_w1*1.2, f'Original: {W1opt[0]:.5f}', 
         verticalalignment='bottom', horizontalalignment='right', fontsize=FONT_SIZE_ANNOTATION, 
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
ax1.text(len(W1optPlot)*0.12, final_value_w1*0.7, f'Optimized: {W1opt[-1]:.5f}', 
         verticalalignment='top', horizontalalignment='right', fontsize=FONT_SIZE_ANNOTATION,
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
...
# Add a secondary y-axis for constraint violation
ax1_cv = ax1.twinx()
ax1_cv.plot(x_values, W1CV, color=ibm_colors["Magenta 40"], linestyle='-', linewidth=2, alpha=0.7, label='Constraint Violation', zorder=1)
ax1_cv.set_ylabel('Constraint Violation', fontsize=FONT_SIZE_AXIS_LABEL, color=ibm_colors["Magenta 40"])
ax1_cv.tick_params(axis='y', which='major', labelsize=FONT_SIZE_TICK_LABELS, colors=ibm_colors["Magenta 40"])
ax1_cv.spines['right'].set_color(ibm_colors["Magenta 40"])
ax1_cv.set_ylim(-.005, .1)
...
for i, val in enumerate(ZIWoptPlot):
    idx = i + 1  # Adjust for 1-based indexing in plot
    if ZIWCV[i] > 0:
        ax2.plot(idx, val, 'ro', markersize=MARKER_SIZE, zorder=3, markeredgecolor=ibm_colors["Magenta 40"], fillstyle='none')
    else:
        ax2.plot(idx, val, 'o', markersize=MARKER_SIZE, markeredgecolor='darkgreen', zorder=3, fillstyle='none')
...
ax2.text(len(ZIWoptPlot)*0.85, initial_value_ziw*1.2, f'Original: {ZIWopt[0]:.5f}', 
         verticalalignment='bottom', horizontalalignment='right', fontsize=FONT_SIZE_ANNOTATION,
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
ax2.text(len(ZIWoptPlot)*0.8, final_value_ziw*0.75, f'Optimized: {ZIWopt[-1]:.5f}', 
         verticalalignment='top', horizontalalignment='right', fontsize=FONT_SIZE_ANNOTATION,
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
ax2.text(1.25, initial_value_ziw*5, f'ZIW', 
         verticalalignment='bottom', horizontalalignment='left', fontsize=FONT_SIZE_TITLE, 
         bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=2))
...
# Add a secondary y-axis for constraint violation
ax2_cv = ax2.twinx()
ax2_cv.plot(x_values, ZIWCV, color=ibm_colors["Magenta 40"], linestyle='-', linewidth=2, alpha=0.7, label='Constraint Violation', zorder=1)
ax2_cv.set_ylabel('Constraint Violation', fontsize=FONT_SIZE_AXIS_LABEL, color=ibm_colors["Magenta 40"])
ax2_cv.tick_params(axis='y', which='major', labelsize=FONT_SIZE_TICK_LABELS, colors=ibm_colors["Magenta 40"])
ax2_cv.tick_params(axis='y', which='minor', labelsize=FONT_SIZE_TICK_LABELS, colors=ibm_colors["Magenta 40"])
ax2_cv.spines['right'].set_color(ibm_colors["Magenta 40"])
ax2_cv.set_ylim(-0.01, .4)
# Make the plot wider
ax2.set_box_aspect(FIGURE_ASPECT_RATIO)
...
plt.show()
# %%
ks = np.linspace(np.pi/2,np.pi,25)/2/np.pi
print(ks[6],ks[14],ks[20])
# %%
