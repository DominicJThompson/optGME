...
# Create a new figure for a single perturbed circle in the center
fig_closeup, ax_closeup = plt.subplots(figsize=(5,5),dpi=200)

# Parameters for the single circle
N = 500  # Number of points around the circle
theta = np.linspace(0, 2*np.pi, N, endpoint=True)
r_decay = 1    # Decay rate parameter
r = .3
sigma = 0.05   # Example standard deviation for noise
lp = 0.5       # Example correlation length
...
# Plot the unperturbed circle as a dashed red circle
unperturbed_circle = plt.Circle((x_center, y_center), r, color=ibm_colors['Magenta 40'], fill=False, linestyle='--', linewidth=3)
ax_closeup.add_patch(unperturbed_circle)

# Set equal aspect ratio and limits
ax_closeup.set_aspect('equal')
margin = 1.2 * r
ax_closeup.set_xlim(-margin, margin)
ax_closeup.set_ylim(-margin, margin)
...
# Draw a short line at the edge of the circle to indicate the standard deviation (sigma)
import matplotlib.patches as mpatches

# Choose the angle (in radians) where the sigma bar should be drawn
theta_sigma = -np.pi / 4  # For example, 45 degrees; change as desired

# Position for the sigma bar (on the circle at angle theta_sigma)
x0 = (r-sigma) * np.cos(theta_sigma)
y0 = (r-sigma) * np.sin(theta_sigma)

# The bar should point radially outward from the circle
# The bar goes from (x0, y0) to (x0 + sigma * cos(theta_sigma), y0 + sigma * sin(theta_sigma))
x1 = x0
y1 = y0
x2 = x0 + 2*sigma * np.cos(theta_sigma)
y2 = y0 + 2*sigma * np.sin(theta_sigma)

# Draw the main sigma bar
ax_closeup.plot([x1, x2], [y1, y2], color=ibm_colors["Purple 40"], linewidth=3, zorder=10)

# Draw perpendicular end bars
end_bar = sigma * r
# The perpendicular direction is rotated by 90 degrees from the radial direction
perp_dx = -np.sin(theta_sigma)
perp_dy = np.cos(theta_sigma)

# At start
ax_closeup.plot(
    [x1 + perp_dx * end_bar, x1 - perp_dx * end_bar],
    [y1 + perp_dy * end_bar, y1 - perp_dy * end_bar],
    color=ibm_colors["Purple 40"], linewidth=3, zorder=11
)
# At end
ax_closeup.plot(
    [x2 + perp_dx * end_bar, x2 - perp_dx * end_bar],
    [y2 + perp_dy * end_bar, y2 - perp_dy * end_bar],
    color=ibm_colors["Purple 40"], linewidth=3, zorder=11
)

# Add sigma label inside the bar (radially in towards the center of the circle)
label_x = x1 - np.cos(theta_sigma) * 2.5 * end_bar
label_y = y1 - np.sin(theta_sigma) * 2.5 * end_bar+.03
ax_closeup.text(label_x, label_y, r'$\sigma$', fontsize=35, ha='center', va='top', zorder=12, color=ibm_colors["Purple 40"])

# Draw an arc along the circle to represent the correlation length (l_p)
# Now, use theta_start and theta_end to define the arc
theta_start = np.pi/16  # starting angle
theta_end = np.pi/4  # for example, or set theta_end directly

# Number of points to draw the arc smoothly
n_arc = 100
arc_thetas = np.linspace(theta_start, theta_end, n_arc)
arc_radius = r * 0.9  # you can change this to r + offset if you want the arc outside the circle

arc_x = arc_radius * np.cos(arc_thetas)
arc_y = arc_radius * np.sin(arc_thetas)

ax_closeup.plot(arc_x, arc_y, color=ibm_colors["Orange 40"], linewidth=3, zorder=11)

# Add perpendicular lines at the ends of the arc
end_bar_len = 0.01  # length of the perpendicular bars

# Start of arc
theta0 = theta_start
x0 = arc_radius * np.cos(theta0)
y0 = arc_radius * np.sin(theta0)
# Tangent direction to the arc at start (perpendicular to radius)
tan_dx0 = np.cos(theta0)
tan_dy0 = np.sin(theta0)
ax_closeup.plot(
    [x0 + tan_dx0 * end_bar_len, x0 - tan_dx0 * end_bar_len],
    [y0 + tan_dy0 * end_bar_len, y0 - tan_dy0 * end_bar_len],
    color=ibm_colors["Orange 40"], linewidth=3, zorder=12
)

# End of arc
theta1 = theta_end
x1 = arc_radius * np.cos(theta1)
y1 = arc_radius * np.sin(theta1)
tan_dx1 = np.cos(theta1)
tan_dy1 = np.sin(theta1)
ax_closeup.plot(
    [x1 + tan_dx1 * end_bar_len, x1 - tan_dx1 * end_bar_len],
    [y1 + tan_dy1 * end_bar_len, y1 - tan_dy1 * end_bar_len],
    color=ibm_colors["Orange 40"], linewidth=3, zorder=12
)

# Place the label at the middle of the arc
label_theta = (theta_start + theta_end) / 2
label_x = arc_radius * 0.85 * np.cos(label_theta)
label_y = arc_radius * 0.85 * np.sin(label_theta) + 0.03  # small offset for clarity

ax_closeup.text(label_x, label_y, r'$l_p$', fontsize=35, ha='center', va='top', zorder=12, color=ibm_colors["Orange 40"])
...
# Add vertical dashed line in the center of both plots
for ax in [ax1, ax2]:
    ax.axvline(x=0, color='k', linestyle='--', linewidth=2, zorder=20)
...
r0_ziw = 0.105
r1_ziw = 0.235
ax2.plot([-2*np.sqrt(3)/3, -2*np.sqrt(3)/3], [-0.5, -0.5+r0_ziw], 'k-', linewidth=2)
ax2.plot([-np.sqrt(3)/3, -np.sqrt(3)/3], [-0.5, -0.5+r1_ziw], 'k-', linewidth=2)
ax2.text(-2*np.sqrt(3)/3, -0.8, '$r_0$', fontsize=22, ha='center',color='k')
