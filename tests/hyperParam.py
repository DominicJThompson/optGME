...
def worker_function(input):
    ...
    #run minimization
    minim = optomization.TrustConstr(vars,W1,cost,mode=20,maxiter=10,gmeParams=gmeParams,constraints=manager,path=input['path'],initial_tr_radius=.1,**input['tcParams'])
    minim.minimize()
    minim.save(input['path'])
#%%
if __name__=='__main__':
    variables = {
                "initial_constr_penalty": np.logspace(-5, -1, num=5),
                "initial_barrier_parameter": np.logspace(-5, -1, num=5),
                "initial_barrier_tolerance": np.logspace(-5, -1, num=5),
                }
    ...
    for i,c in enumerate(combinations):
        input = {'path':f"tests/media/hypeTest{i}.json",'tcParams':c}
